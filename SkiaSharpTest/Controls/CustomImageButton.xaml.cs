...
namespace SkiaSharpTest.Controls
{
    ...
    {
        private static SKBitmap? s_buttonNormal;
        private static SKBitmap? s_buttonSelected;
        private static SKBitmap? s_buttonDisabled;

        static CustomImageButton()
        {
            s_buttonNormal = LoadBitmap("button_normal.png");
            s_buttonSelected = LoadBitmap("button_selected.png");
            s_buttonDisabled = LoadBitmap("button_disabled.png");
        }
        ...
        public CustomImageButton()
        {
            ...
            customButton.Pressed += (object? sender, EventArgs args) =>
            {
                ...
                if (UseVaryingTextColors)
                    customButton.TextColor = !IsEnabled ? DisabledTextColor : SelectedTextColor;
                customCanvasView.InvalidateSurface();
            };
            customButton.Released += (object? sender, EventArgs args) =>
            {
                IsPressed = false;
                if (UseVaryingTextColors)
                    customButton.TextColor = !IsEnabled ? DisabledTextColor : NormalTextColor;
                customCanvasView.InvalidateSurface();
            };
            ...
            {
                if (customCanvasView.Handler?.PlatformView is SkiaSharp.Views.Windows.SKXamlCanvas canvas)
                {
            ...
            {
                if (customButton.Handler?.PlatformView is Microsoft.UI.Xaml.Controls.Button button)
                {
        ...
        public static readonly BindableProperty TextColorProperty = BindableProperty.Create(nameof(TextColorProperty), typeof(Color), typeof(CustomImageButton), Colors.White, propertyChanged: OnTextColorChanged);
        ...
        {
            if (bindable is CustomImageButton cib && newValue is Color newColor)
            {
                cib.customButton.TextColor = newColor; // simplified since we don't have UIUtils here
            }
        }
        ...
        private void CustomCanvasView_PaintSurface(object? sender, SKPaintSurfaceEventArgs e)
        {
...
#if WINDOWS
            SKBitmap? targetBitmap = !UseVaryingBackgroundImages ? s_buttonNormal : IsPressed && IsEnabled ? s_buttonSelected : _isHovering && IsEnabled ? s_buttonNormal : s_buttonDisabled;
#else
            SKBitmap? targetBitmap = !UseVaryingBackgroundImages ? s_buttonNormal : !IsEnabled ? s_buttonDisabled : IsPressed ? s_buttonSelected : s_buttonNormal;
#endif
            ...
            {
                canvas.DrawBitmap(targetBitmap, sourcerect, targetrect, paint);
            }
        }
    }
}
